...
student_id = "202210179018"
last_digit = int(student_id[-1])  # 提取最后一位数字

nationality_name = mapping[last_digit]
nationality_index = vectorizer.nationality_vocab.lookup_token(nationality_name)

print(f"Sampled for {nationality_name}: ")
sampled_indices = sample_from_model(
    model,
    vectorizer,
    nationalities=[nationality_index] * 5,  # 生成5个相同国家
    temperature=0.7
)
